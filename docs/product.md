...
**Pedidos iFood na fila/detalhe (modo externo, #129):** `Order(source=IFOOD)` aparece na fila e no filtro por origem; pagamento “Pago/gerenciado pelo iFood”; nota de status controlado pelos eventos; sem botões locais de status/recebimento (incluindo `KITCHEN`). Actions iFood pelo painel ficam para a fatia seguinte. Detalhe: [product/ifood-admin-queue-readonly.md](product/ifood-admin-queue-readonly.md).
