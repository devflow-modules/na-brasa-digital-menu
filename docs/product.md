...
**Pedidos iFood na fila/detalhe (modo externo, #129):** `Order(source=IFOOD)` aparece na fila e no filtro por origem; pagamento “Pagamento gerenciado pelo iFood”; nota de status controlado pelos eventos. Detalhe: [product/ifood-admin-queue-readonly.md](product/ifood-admin-queue-readonly.md).

**Actions iFood no detalhe (#131):** um botão da próxima ação (`CONFIRM` / `START_PREPARATION` / `READY_TO_PICKUP`|`DISPATCH`) resolve no servidor a partir do `Order.id` interno e delega ao command ledger; HTTP 202 aguarda evento; sem mutação otimista do status. RBAC reutiliza permissões de status existentes (`KITCHEN` sem confirm/dispatch). Detalhe: [product/ifood-admin-order-actions.md](product/ifood-admin-order-actions.md).
