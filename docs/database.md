...
- **iFood inbox (test app, #120)** — `IfoodConnection` (`Store` ↔ `merchantId`; `isActive=false` para o poller), `IfoodEvent` append-only (`@@unique([connectionId, externalEventId])`), `IfoodOrder` snapshot por `externalOrderId` com lifecycle por `lastEventAt` + desempate `lastExternalEventId`. Credenciais só em env.
- **iFood actions (test app, #122)** — `IfoodOrderCommand` representa uma ação lógica idempotente por pedido/tipo; `PENDING`/`ACCEPTED` não confirmam lifecycle. Só o evento correlacionado muda para `CONFIRMED`. `IfoodOrderCommandAttempt` audita tentativas e status HTTP sanitizado, sem corpo upstream/PII.
- **iFood → Order projection (#126 PR A)** — `IfoodOrder.operationalOrderId` (unique/FK opcional 1:1) liga ao `Order` operacional (`source=IFOOD`). Criação idempotente a partir do snapshot; status local só por evento iFood; sem `OrderPayment` / UI / webhook nesta fatia. Ver [ifood-order-projection.md](product/ifood-order-projection.md).
