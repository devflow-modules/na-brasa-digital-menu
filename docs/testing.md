...
| `tests/e2e/admin-status.spec.ts` | PICKUP: PENDING → … → COMPLETED (MANAGER) |
...
- `src/features/admin/settings/admin-store-settings.schema.test.ts` (mínimo entrega, modalidades, teto monetário)
- `src/features/admin/settings/admin-store-settings.service.test.ts` (permissão / rejeição pré-persistência)
